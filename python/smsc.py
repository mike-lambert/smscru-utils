...
# MikeLambert@github: library was extended to synchronous command-line utility
...
if __name__ == "__main__":
    mode_map = {'text' : 0, 'hex' : 5, 'cost' : -1, 'hlr' : 3, 'balance': -1, 'ping': 6};
    mode = argv[1];
    f = mode_map[mode];
    retries = 10;
    smsc = SMSC();
    ...
    if mode == "text" or mode == "hex" or mode == "cost" and len(argv) >= 4:
        payload = argv[3];
        if mode == "cost":
            v = smsc.get_sms_cost(enum, payload);
            print('x' + v[1] + ' SMS;' + ('%.4f' % float(v[0])) + ' RUR');
        else:
            v = smsc.send_sms(enum, payload, format = f);
            id = v[0];
            while True:
                v = smsc.get_status(id, enum, all = 0);
                if int(v[0]) > 0:
                    break;
                elif retries == 0:
                    v[0] = '503';
    ...
    elif mode == "hlr" or mode == "ping":
        v = smsc.send_sms(enum, "", format = f);
        id = v[0];
        while True:
            v = smsc.get_status(id, enum, all = 0);
            if int(v[0]) > 0:
                ...
                sleep(3);
                retries = retries-1;
        if int(v[0]) == 1 and mode == "hlr":
            print(v[5] + v[6] + ';' + v[7] + ';' + v[8] + ';' + v[4] + ';' + v[3]);
        elif int(v[0]) == 1 and mode == "ping":
            print('OK');
        else:
            print('Ошибка ' + v[0]);
    else:
        print "Usage: " + argv[0] + " balance                       - show account balance";
        print "       " + argv[0] + " hlr|ping <phone>              - retrieve HLR info or ping for <phone>";
        print "       " + argv[0] + " cost <phone> <text>           - compute transmission cost to <phone> according message <text>";
        print "       " + argv[0] + " text|hex <phone> <text> - sending to <phone> text, or HEX-encoded binary SMS with <text> payload respectively";
